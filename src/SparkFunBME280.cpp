...
BME280::BME280(void)
{
    ...
    settings.commInterface = kSfeI2CMode; // Default to I2C
...
uint8_t BME280::begin()
{
    ...
    case kSfeI2CMode:
        ...
        // Removing port begin from library. This should be done by user otherwise this library will overwrite Wire
        // settings such as clock speed.
        //  switch(_wireType)
        //  {
        //  	case(HARD_WIRE):
        //  		_hardPort->begin(); //The caller can begin their port and set the speed. We just confirm it here
        //  otherwise it can be hard to debug. 		break; 	case(SOFT_WIRE): 	#ifdef SoftwareWire_h
        //  _softPort->begin(); //The caller can begin their port and set the speed. We just confirm it here otherwise
        //  it can be hard to debug. 	#endif 		break;
        //  }
        break;
    ...
    calibration.dig_H4 =
        ((int16_t)((readRegister(BME280_DIG_H4_MSB_REG) << 4) + (readRegister(BME280_DIG_H4_LSB_REG) & 0x0F)));
    calibration.dig_H5 =
        ((int16_t)((readRegister(BME280_DIG_H5_MSB_REG) << 4) + ((readRegister(BME280_DIG_H4_LSB_REG) >> 4) & 0x0F)));
...
bool BME280::beginSPI(uint8_t csPin, SPIClass &spiPort)
{
    ...
    settings.commInterface = kSfeSPIMode;

    uint8_t chipID = begin();

    if (chipID == 0x58)
        return (true); // Begin normal init with these settings. Should return chip ID of 0x58 for BMP
    if (chipID == 0x60)
        return (true); // Begin normal init with these settings. Should return chip ID of 0x60 for BME
    return (false);
}
...
bool BME280::beginI2C(TwoWire &wirePort)
{
    ...
    settings.commInterface = kSfeI2CMode;
...
float BME280::readFloatAltitudeMeters(void)
{
    ...
    heightOutput = ((float)-44330.77) * (pow(((float)readFloatPressure() / (float)_referencePressure), 0.190263) -
...
uint8_t BME280::readRegister(uint8_t offset)
{
    ...
    case kSfeI2CMode:
        ...
        case (HARD_WIRE):
            ...
            bytesAvailable = _hardPort->requestFrom(settings.I2CAddress, numBytes);
            if (bytesAvailable == numBytes)
            {
                result = _hardPort->read();
            }
            break;
        ...
        case (SOFT_WIRE):
            ...
            bytesAvailable = _softPort->requestFrom(settings.I2CAddress, numBytes);
            if (bytesAvailable == numBytes)
            {
                result = _softPort->read();
            }
#endif
            break;
        }
